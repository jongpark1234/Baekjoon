MOD = 1_000_000_007
def __resize(numlist: list[int], idx: int) -> list[int]:
...
def __multiply(v1: list[int], v2: list[int], rec: list[int]) -> list[int]:
    ...
    마지막으로 __resize 함수를 사용하여 리스트의 길이를 맞춰줍니다."""
    vLength = len(v1)
    ...
    for i in range((vLength << 1) - 1, vLength - 1, -1):
        for j in range(1, vLength + 1):
            ...
            if t[i - j] >= MOD:
                t[i - j] -= MOD
    return __resize(t, vLength) # 리스트의 길이를 조절하고 반환
def __berlekampMassey(numlist: list[int]) -> list[int]:
    ...
    for i in range(len(numlist)):
        ...
        for j in range(len(cur)):
            t = (t + numlist[i - j - 1] * cur[j]) % MOD # 현재 다항식을 이용하여 t를 계산한다.
        if (t - numlist[i]) % MOD == 0: # t가 현재 수열 numlist[i]와 같으면 다음 수열 값을 계산
            continue
        if not cur: # 현재 다항식이 비어있다면 초기화
            cur = __resize(cur, i + 1)
            lf, ld = i, (t - numlist[i]) % MOD # lf: 마지막으로 저장된 위치, ld: 마지막 계수
            continue
        k = -(numlist[i] - t) * pow(ld, MOD - 2, MOD) % MOD # 다음 계수 k 계산
        c = [0 for _ in range(i - lf - 1)] + [k] + [-j * k % MOD for j in ls] # 다항식을 계산한다.
        if len(c) < len(cur):
            c = __resize(c, len(cur))
        for j in range(len(cur)):
            c[j] = (c[j] + cur[j]) % MOD # 다음 다항식을 계산한다.
        if i - lf + len(ls) >= len(cur): # 현재 다항식의 크기보다 크다면 업데이트
            ls, lf, ld = cur, i, (t - numlist[i]) % MOD
            # 마지막으로 저장된 다항식을 업데이트하고, lf와 ld도 업데이트
        cur = c[:] # 다음 다항식을 현재 다항식으로 설정
    return list(map(lambda x: (x % MOD + MOD) % MOD, cur))
def __getNth(rec: list[int], numlist: list, idx: int) -> int:
    ...
    while idx:
        if idx & 1:
            s = __multiply(s, t, rec) # s에 변환 행렬 t를 곱해 갱신
        t = __multiply(t, t, rec) # t를 제곱하여 갱신
        idx >>= 1 # idx를 오른쪽으로 1비트 이동
    return sum(s[i] * numlist[i] % MOD for i in range(m)) % MOD # s와 numlist를 곱한 값의 합을 반환
def process(numlist: list[int], idx: int) -> int:
    return numlist[idx] if idx < len(numlist) else 0 if not (v := __berlekampMassey(numlist)) else __getNth(v, numlist, idx)
...
print(n if k == 0 else process(result, n))
